    ...
    Axios.post("/api/url/create", {
      ...
      .then((response) => {
        switch (response.status) {
          case 200:
            isSuccess = true;
            msg = "A short URL is successfully generated for this long URL!";
            data = response.data;
            break;
          case 201:
            isSuccess = true;
            data = response.data;
            msg = "Someone has already created a short URL for this website!";
            break;
          default:
            isSuccess = false;
            msg = errMsg;
        ...
        if (err.response) {
          switch (err.response.status) {
            case 401:
              isSuccess = false;
              console.log("401!");
              msg = "This website's URL is not valid! Please try again!";
              break;
            case 409:
              isSuccess = false;
              msg =
        ...
        <Form>
          ...
          <Form.Group controlId="formCustomString">
            ...
            <Row className="ml-0 mb-2">
              <Button
                id="custom-btn"
                className={"btn-info mr-3 " + (customMode ? "" : "disabled")}
                onClick={() => {
                  setCustomMode(true);
                }}
            ...
            <Collapse in={customMode}>
              <div id="input-custom-string">
                <Form.Label>Set your own custom label*</Form.Label>
            ...
            <Row className="mt-4">
              <Button
                ...
                variant="primary"
                disabled={
                  !((!customMode && url) || (customMode && url && customInput))
                }
                onClick={() => createUrl()}
              >
                Create
